...
class Penjualan extends CI_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(['barang_model', 'penjualan_model', 'perusahaan_model']);
        if (!is_logged_in()) {
    ...
    public function index()
    {
        ...
        $data['list_perusahaan'] = $this->perusahaan_model->getAll();
    ...
    public function add()
    {
        ...
        if ($isAlreadyItem) {
            ...
            if ($barang[0]->stok < $data['qty'] && $barang[0]->stok <> 0) {
                $query = false;
                $result['error_message'] = 'Data Qty transaksi melebihi Qty stok yang tersedia.';
            } elseif ($barang[0]->stok > 0) {
                $query = $this->penjualan_model->addKeranjang($data);
            } else {
                $result['error_message'] = 'Data stok toko 0 silahkan tambahkan stok baru.';
                $query = false;
            }
        }
